                         ...
                         @foreach ($quotes as $row => $quoteRow)
                             ...
                             @foreach ($quoteRow->shuffle() as $key => $quote)
                                 <div x-show="currentRow == {{ $row }}"
                                     ...
                                     class="{{ ($key % 2) - ($row % 2) ? 'col-start-2' : 'col-start-1' }} row-start-{{ $key + 1 }} type-xs lg:type-sm relative col-span-1 flex h-24 items-center justify-center px-1 text-center !leading-none lg:h-32 lg:px-4">
                                     {{ $quote->quote }}
                                     @svg('bubble-' . $bubbles[$key], 'absolute  -z-10 -left-3 w-full lg:w-[calc(100%+1.5rem)] -right-3 h-auto top-1/2 -translate-y-1/2 max-w-none')
                                 </div>
                             @endforeach
                         @endforeach
                     </div>
