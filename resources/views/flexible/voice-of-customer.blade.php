 @if ($layout->quotes)
     ...
     <div x-data="{
     ...
     }" class="container mx-auto max-w-6xl py-16"
         x-intersect="if(!initialised) { initialised = true; initialise()}">

         @include('components.block-intro', ['layout' => $layout])

         <template x-if="shown">
             <div class="my-16 grid grid-cols-2 gap-y-8 px-0 lg:gap-y-0">
                 @foreach ($quotes as $row => $quoteRow)
                     @php($bubbles = collect([1, 2, 3, 4])->shuffle())
                     @foreach ($quoteRow->shuffle() as $key => $quote)
                         <div x-show="currentRow == {{ $row }}"
                             x-transition:enter="transition ease-out duration-[1500ms] delay-[{{ 1000 + $bubbles[$key] * 250 }}ms]"
                             ...
                             class="{{ $key % 2 ? 'col-start-2' : 'col-start-1' }} {{ $key < 2 ? 'row-start-1' : 'row-start-2' }} {{ $bubbles[$key] == 4 || $bubbles[$key] == 1 ? 'text-white' : null }} {{ (($row + 1) % 2 && $key > 1) || ($row % 2 && $key < 2) ? 'ml-auto' : null }} relative col-span-1 flex aspect-square w-3/4 items-center justify-center p-2 text-center font-bold leading-[1.1] lg:w-1/2 lg:p-4 lg:text-3xl lg:leading-tight">
                             {{ $quote->quote }}
                             @svg('bubble-' . $bubbles[$key], 'absolute -inset-4 lg:-inset-8 -z-10 w-[calc(100%+2rem)] h-[calc(100%+2rem)] lg:w-[calc(100%+4rem)] lg:h-[calc(100%+4rem)]')
                         </div>
                     @endforeach
                 @endforeach
             </div>
         ...
         <div class="mx-auto flex max-w-xl flex-row items-center rounded-3xl bg-light-teal bg-opacity-20 pl-8">

             <svg class="h-auto w-32" xmlns="http://www.w3.org/2000/svg" width="104.23" height="121.75"
                 viewBox="0 0 104.23 121.75">
                 <path fill="#fff"
                 ...
                 <g fill="none" stroke="#0f1115" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5">
