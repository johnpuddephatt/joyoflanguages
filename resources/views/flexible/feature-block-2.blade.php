<div
    ...
    <div class="{{ $class ?? 'mx-auto' }} container">
        <div
            class="{{ $layout->colour ?? 'bg-teal-light' }} {{ $layout->images ? ' pb-24 lg:pb-48' : null }} relative flex flex-col gap-4 rounded-3xl p-8 lg:flex-row lg:gap-8 lg:p-16">
            ...
            @endif
            <img class="w-2/3 lg:w-2/5" src="{{ Storage::url($layout->image) }}" />

            <div
                class="{{ $layout->reverse ? 'lg:order-first' : null }} relative z-10 flex flex-1 flex-col items-start justify-center lg:px-8 lg:py-8">
                <h2 class="max-w-lg text-2xl font-bold lg:mb-6 lg:text-4xl">{!! $layout->title !!}</h2>
                @if ($layout->subtitle)
        ...
        @if ($layout->images)
            <div class="{{ $class ?? 'mx-auto max-w-7xl' }} relative -mt-36 flex flex-row gap-16 px-24">
                <svg class="absolute bottom-1/2 left-1/2 h-auto w-screen max-w-none -translate-x-1/2"
