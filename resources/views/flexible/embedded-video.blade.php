<div id="{{ $layout ? $layout->key() : null }}"
    class="{{ $layout->background_colour }} relative pb-8 pt-16 lg:pt-24 2xl:pb-16 2xl:pt-32">
    ...
    <div class="{{ $class ?? 'mx-auto max-w-6xl' }} relative z-[21] lg:container">
        @if ($layout->sticker)
            <div class="absolute left-4 w-32 -translate-y-1/3 lg:right-full lg:w-48 lg:translate-x-1/4">
                ...
                <div
                    class="absolute inset-0 z-10 flex max-w-full flex-col items-center justify-center p-4 text-center font-bold !leading-tight lg:text-xl">
                    {{ $layout->sticker }}</div>
            </div>
        @endif
