        ...
        <div class="mt-12 flex flex-col border-b border-teal lg:mt-16" x-data="{ current: null }">
            @foreach ($layout->faqs as $faq)
                <details x-transition x-show="{{ $loop->index }} < showing"
                    ...
                    <summary
                        @click.prevent="current = (current == {{ $loop->index }}) ? null : {{ $loop->index }}; $el.scrollIntoView({ behavior: 'smooth', block: 'start' })"
                        class="cursor-pointer scroll-pt-24 px-2 py-4 text-xl font-semibold transition hover:bg-light-teal hover:bg-opacity-10">
